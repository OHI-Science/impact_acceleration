...
all_stressors_formatted <- all_stressors %>%
  ...
  select(KNBpackage, year, data_stage, filepath, new_file_name)                              
...
habitat_data <- data.frame(KNBpackage = "Cumulative Impact", year = NA, data_stage = "habitat", filepath=habitat_files, habitat=basename(habitat_files)) %>%
mutate(new_file_name = sprintf("%s_%s", data_stage, habitat)) %>%
  select(KNBpackage, year, data_stage, filepath, new_file_name) 
...
final_list <- rbind(all_stressors_formatted, cumulative_impacts, habitat_data) %>%
