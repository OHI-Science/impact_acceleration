...
Stressor impacts, $I_{s}$, were calculated for each of the 14 stressors by first multiplying the stressor instensity raster, $D_{j}$, by each ecosystem raster, $E_{i}$, and the corresponding vulnerability value, $mu _{ij}$. The stressor x ecosystem x vulnerability rasters are summed for each stressor and then divided by the number of habitats ($m$) in each raster cell:
...
All data are represented at ~1km2 resolution with a WGS84 Mollweide projection. All data were resampled/reprojected using nearest neighbor estimates of cell values. Nearly all of the source data used to derive the stressor layers had native coarser resolution (Table 2) than our representation.  Using the nearest neighbor approach preserves the values of the original data, and assumes the coarse-scale value is
evenly distributed across all 1km2 cells within that region. The coarser scale pattern is essentially recreated while the finer resolution information is preserved where and when it is appropriate. 

We use the WGS84 Mollweide projection because it preserves area, and data towards the poles are not visually over-represented.
...
Table 1. Gear types provided by Watson (2018) used to assign catch to each commercial fisheries pressure 
...
The sea surface temperature (SST) stressor describes the increased frequency of extreme temperature events relative to a historical baseline period.  We used weekly SST anomoly data from version 6 of the Coral Reef Temperature Anomaly Database (CoRTAD) These data are a product of the NOAA National Oceanographic Data Center, produced using Advanced Very High Resolution Radiometer (AVHRR) Pathfinder SST data (https://data.nodc.noaa.gov/cgi-bin/iso?id=gov.noaa.nodc:NCEI-CoRTADv6). SST anomolies are defined as the weekly SST minus weekly climatological SST, as calculated across all years of data (1982-2017).

The number of weeks, during a five year period, with extremely warm SST was subtracted from the number of extreme SST weeks during a five year baseline period (1985-1989). An extreme SST event was defined as a week when the SST anomoly exceeded the standard deviation of SST anomolies for that location (i.e., grid cell) and week of the year. Extreme events included only high (i.e., warming) SST events.  
...
There were some coastal raster cells without data.  For these cells, we estimated missing 5 year average sea level rise anomolies using an inverse distance weighting method.  Red areas in the following figure indicate gapfilled cells.
...
For each year, the original raster values of 0-63 ($DNo$) were calibrated using data only from the Sicily region (which has had a fairly stable population during this period) to produce adjusted, $DNa$, values using the following formula:

$$DNa = C0 + C1*DNo + C2*DNo^2$$
 
 
C0, C1 and C1 coefficients used to calibrate the data were estimated for each year using a 2nd degree polynomial regression model, with the F121999 satellite as a predictor variable, with the model coded in R as: 
...
*Table 2: Data sources for stressor data*
...
Shipping | Halpern et al. 2015 (high resolution shipping data); Tournadre 2018 (low resolution annual shipping data) | Tournadre, ships per cell, such that sum of cells represents all ships at sea in given year; Halpern et al., vessel track density | Halpern: 0.1&deg;; Tournadre: 1x2&deg; | Halpern: combined years (1991-2011, but mostly 2011); Tournadre: yearly, 1992-2016



*Table 3: Methods used for stressor data*
...
sea level rise     | Monthly anomolies averaged to obtain annual mean sea level anomoly.  5 year mean of annual data used to smooth large yearly variation.| estimated missing coastal data using inverse distance weighting of 5 year mean anomoly data | raster value corresponding 99.99th quantile across all years | NA
...
direct human | Density (people per km2) converted to population (people per raster cell). Intervening years (i.e., years outside of 2000, 2005, 2010, 2015, 2020) interpolated using a linear model.  For each raster cell, soastal human population summed for 10 km radius. Data cropped to include only cells 1km from the coast.  | NA | raster value corresponding to average 99.99th quantile across all years | ln(x + 1)
light | Non-calibrated radiance values from satellite data calibrated across year/satellite following methods of Elvidge et al. 2009. | NA | raster value corresponding 99.99th quantile across all years (excluding 0 values) | NA
...
The vulnerability matrix is used to translate stressor data into impact data.  This table describes the impact of each stressor on each marine ecosystem, using integers ranging from 0 (no impact) to 3 (highest impact). These weighting variables are from an expert survey that assessed the vulnerabiliyt of each ecosystem to each stressor on the basis of 5 ecological traits (Halpern et al. 2008).  The vulnerability matrix has remained unchanged from the 2015 cumulative impact analysis (Halpern et al. 2015). 
