...
writeRaster(catch_gf_mol_mask, file.path(dir_M,'/git-annex/impact_acceleration/stressors/art_fish/int/art_fish_gf.tif'), overwrite=TRUE)
...
foreach(file = annual_files, .packages="dplyr") %dopar% { # file = annual_files[65]
  ...
  data <- readRDS(file) %>%
      ...
      dplyr::summarise(cell_catch = sum(SSF_CR)) %>%
...
for(year_catch in 1950:2014){ # year_catch = 2014
