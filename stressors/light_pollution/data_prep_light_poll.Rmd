---
title: 'OHI Light Pollution data'
...
This script uses light data collected from DMSP satellites.
...
Data comes from [NOAA Earth Observation Group](https://ngdc.noaa.gov/eog/dmsp/downloadV4composites.html)
...
This formula is based on the assumption that a reference area (Sicily) has not changed much in light output in recent years. Lowest values in DNa (non-lit areas) were replaced with zeros. This technique, while not perfect, does allow for better comparison between different satellites and time periods.
...
#### Get the corretion factors based on Sicily data
Elvidge et al., 2009:
...
One thing that I add is that I force the intercept through zero so the zero cells remain zero.
...
for(file in data_files){ #file = data_files[1]
  ...
  plot(correct$F121999, correct[ ,loc], main=file)
...
raster::raster(rast) %>%
        raster::calc(fun=function(x){ifelse(x>0, 
                                            ...
                                            0)}, 
                     filename = file.path(dir_M, sprintf("git-annex/impact_acceleration/stressors/light_pollution/final/light_%s_recaled_mol.tif", yr)), overwrite=TRUE)
}