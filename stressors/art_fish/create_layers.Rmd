...
art_catch_rasts <- list.files(file.path(dir_M,'git-annex/impact_acceleration/stressors/art_fish/int/catch_annual_rasters'), full.names=TRUE, pattern = "gf")
art_catch_rasts <- art_catch_rasts[grep(years_filter, art_catch_rasts)]

registerDoParallel(6) #register parallel cores

foreach(file = art_catch_rasts) %dopar%{ # file = art_catch_rasts[1]
...
writeRaster(c, filename = file.path(dir_M, sprintf('git-annex/impact_acceleration/stressors/art_fish/int/catch_annual_npp_rasters/art_fish_%s.tif', yr)), overwrite=TRUE) 
...
catch_npp <- list.files(file.path(dir_M, 'git-annex/impact_acceleration/stressors/art_fish/int/catch_annual_npp_rasters'), full.names = TRUE)
...
#list of lists
all_files <- list.files(file.path(dir_M, 'git-annex/impact_acceleration/stressors/art_fish/int/catch_annual_npp_rasters'), full.names=TRUE)
...
write.csv(tmp, file.path(dir_M, 'git-annex/impact_acceleration/stressors/art_fish/int/quantile_9999.csv'))


foreach(file = all_files) %dopar% {    # file = all_files[1]
     ...
     raster(file) %>%
         calc(fun=function(x){ifelse(x<0, 0,
          ...
          resample(ocean, method = 'ngb') %>%
            mask(ocean,
            filename = file.path(dir_M, sprintf('git-annex/impact_acceleration/stressors/art_fish/final/art_fish_%s_rescaled_mol.tif', yr)),                       overwrite=TRUE)  
...
results <- list.files(file.path(dir_M,'git-annex/impact_acceleration/stressors/art_fish/final'), full.names = TRUE) %>%
          stack()

plot(results, col=cols, axes=FALSE)
