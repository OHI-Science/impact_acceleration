...
quant <- read.csv(file.path(dir_M, 'git-annex/impact_acceleration/stressors/art_fish/int/quantile_9999.csv'))[["quant_9999"]]
     ...
     raster::raster(file) %>%
         raster::calc(fun=function(x){ifelse(x<0, 0,
          ...
          raster::resample(ocean, method = 'ngb') %>%
            raster::mask(ocean,
            filename = file.path(dir_M, sprintf('git-annex/impact_acceleration/stressors/art_fish/final/art_fish_%s_rescaled_mol.tif', yr)),                       
            overwrite=TRUE)  
...
results <- list.files(file.path(dir_M,'git-annex/impact_acceleration/stressors/art_fish/final'), full.names = TRUE)[12]
