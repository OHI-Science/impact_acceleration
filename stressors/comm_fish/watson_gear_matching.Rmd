...
#devtools::install_github("hadley/tidyverse") updated tidyverse pckgs
```
...
raw <- read_csv(file.path(dir_M,'git-annex/impact_acceleration/stressors/comm_fish_v3.0/data/Index.csv'))

#look at all the unique gear types

gear <- raw %>%
          dplyr::select(FleetGearName)%>%
          unique() %>% 
          arrange(FleetGearName)
...
The bycatch, type and destruction information was manually added for each gear type using figure S4 from Halpern (2008). Some I left some blank as I don't know if they are demersal or pelagic. This information was saved in `gear_to_cat.csv`.
...
```{r}
data <- raw %>%
        dplyr::select(TaxonName,CommonName, FleetGearName)%>%
        unique() %>%
        left_join(gear_cat,by = c("FleetGearName"))
...
dt_taxon           <- fread(file.path(dir_M,'git-annex/impact_acceleration/stressors/comm_fish_v3.0/data/taxon.dat'), sep=";", header = FALSE)
...
catch_df_taxa <-data %>%
                ...
                unique()%>%
                left_join(dt_taxon %>% 
                            rename(TaxonName = Scientific_Name) %>% 
                            select(-Common_Name),
                          by='TaxonName') %>% 
                select(TaxonKey, everything())

nas <- filter(catch_df_taxa,is.na(FunctionalGroupDescription))
```
...
catch_df_taxa <- catch_df_taxa%>%
                  ...
                  mutate(type = ifelse(str_detect(CommonName, 'marlin|tuna|Tuna|Shark|sharks|Herring|Corvina|Cartilaginous fishes|animals'),'pelagic',                                  ifelse(str_detect(CommonName,'decapods|cucumbers|cockles|Mollusca|molluscs|Sea stars|Starfishes|lobster|urchins'),'demersal',type)))

DT::datatable(catch_df_taxa,rownames = F)
```

```{r}
catch_df_taxa %>% filter(is.na(type))
```

```{r}
catch_df_taxa <- catch_df_taxa %>% 
  mutate(type = case_when(
    is.na(type) & str_detect(CommonName, "venus|Turbot|Black porgy|gurnard|perch|herring|threadfish|needlefish|mullet|ariomma|silverfish|squid|larseni|bream|Snipefishes|bonito|trout") ~ "pelagic",
    is.na(type) & str_detect(CommonName, "shell|rosefish|cockle|seacatfish|Misty|Warsaw|scallop|Yellowedge|Sailray|flounder|dogfish|Snowy|sculpin") ~ "demersal",
    TRUE ~ type
  )) 
```
...
data_plus_gear_info <- data %>%
                        left_join(catch_df_taxa,by = c('CommonName','TaxonName')) %>%
...
mismatch <- data_plus_gear_info%>%
      filter(diff != 1)%>%
      dplyr::select(CommonName,FleetGearName,bycatch,type.x,type.y,destruction,diff)%>%
      unique()

DT::datatable(mismatch,rownames = F)
```

It looks like there are `r nrow(mismatch)` cases where we have mismatched types. After looking at it, it's complicated. What I propose is that when the gear says it is either a bottom trawl, dredge, or trap, it is demersal. If it is a mid-water trawl or a purse seine it is pelagic. 

```{r}
mismatch_fix <- mismatch %>%
  mutate(true_type = case_when(
    str_detect(CommonName, 'Cods|cod|guitarfish|squillid|moray|Lizard') ~ "demersal", # species rules by Jamie
    str_detect(FleetGearName,'Trawl midwater|seine') ~ "pelagic",
    str_detect(FleetGearName,'Trawl|Trap|Dredge') ~ "demersal"))
```
...
df_type_dest <- data_plus_gear_info%>%
              left_join(mismatch_fix,by = c('CommonName','FleetGearName','bycatch','type.y','type.x','destruction','diff'))%>%
              rowwise()%>%
...
** No need for this section anymore. All gear types have bycatch levels**
