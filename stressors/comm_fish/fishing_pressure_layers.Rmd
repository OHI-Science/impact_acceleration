...
foreach (yr = c(2003:2010)) %dopar%{
  ...
  cat_hb <- hb_files[substr(hb_files,90,93) == yr]%>%
              raster()%>%
              projectRaster(n,over=T,method = 'ngb')
...
min_v_hb <- log(min(vals,na.rm=T)+1) # 0
max_v_hb <- log(max(vals,na.rm=T)+1) # 11.32801

ref_hb <- log(quantile(vals,prob = 0.9999,na.rm=T)+1)
...
min_v_lb <- log(min(vals,na.rm=T)+1) #0
max_v_lb <- log(max(vals,na.rm=T)+1) #12.3698


ref_lb <- log(quantile(vals,prob = 0.9999,na.rm=T)+1)
