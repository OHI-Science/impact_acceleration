...
watson_data <- list.files(file.path(dir_M,'marine_threats/impact_acceleration/v2016/stressors/comm_fish/data'),full.names=T)
...
write_rds(yr_data,path = paste0(file.path(dir_M),'/marine_threats/impact_acceleration/v2016/stressors/comm_fish/int/catch_data_',yr,'.rds'))

}
}

mclapply(watson_data,annual_csv,mc.cores = 8) #this was done in parallel to speed up processing time
...
annual_files <- list.files(file.path(dir_M,'marine_threats/impact_acceleration/v2016/stressors/comm_fish/int'),full.names=T)
...
dem_dest_fun <- function(file){
  
  yr <- substr(file,94,97)
  
  data <- readRDS(file)%>%
      filter(destruction == 'destructive',
             fish_type == 'demersal')%>%
      rowwise()%>%
      mutate(catch = sum(LSF_CR,SSF_CR,IUU_CR,Discards_CR,na.rm=T))%>%
  ...
  #rasterize catch by swapping cell ids with 
      raster::subs(saup_rast, data, by = 1, which = 2, subsWithNA=TRUE, filename = paste0(file.path(dir_M),'/marine_threats/impact_acceleration/v2016/stressors/comm_fish/output/annual_catch/dem_dest/dem_dest_',yr,'.tif'),overwrite=T)
}

mclapply(annual_files,dem_dest_fun,mc.cores = 5)
...
dem_nd_hb_fun <- function(file){
  ...
  data <- readRDS(file)%>%
     ...
     raster::subs(saup_rast, data, by = 1, which = 2, subsWithNA=TRUE, filename = paste0(file.path(dir_M),'/marine_threats/impact_acceleration/v2016/stressors/comm_fish/output/annual_catch/dem_nondest_hb/dem_nondest_hb_',yr,'.tif'),overwrite=T)
}

mclapply(annual_files,dem_nd_hb_fun,mc.cores = 4)
```
...
dem_nd_lb_fun <- function(file){
  ...
  data <- readRDS(file)%>%
     ...
     raster::subs(saup_rast, data, by = 1, which = 2, subsWithNA=TRUE, filename = paste0(file.path(dir_M),'/marine_threats/impact_acceleration/v2016/stressors/comm_fish/output/annual_catch/dem_nondest_lb/dem_nondest_lb_',yr,'.tif'),overwrite=T)
}

lapply(annual_files,dem_nd_lb_fun)
...
pel_lb_fun <- function(file){
  ...
  data <- readRDS(file)%>%
     ...
     raster::subs(saup_rast, data, by = 1, which = 2, subsWithNA=TRUE, filename = paste0(file.path(dir_M),'/marine_threats/impact_acceleration/v2016/stressors/comm_fish/output/annual_catch/pel_lb/pel_lb_',yr,'.tif'),overwrite=T)
}


lapply(annual_files,pel_lb_fun)
...
pel_hb_fun <- function(file){
  ...
  data <- readRDS(file)%>%
     ...
     raster::subs(saup_rast, data, by = 1, which = 2, subsWithNA=TRUE, filename = paste0(file.path(dir_M),'/marine_threats/impact_acceleration/v2016/stressors/comm_fish/output/annual_catch/pel_hb/pel_hb_',yr,'.tif'),overwrite=T)
}

lapply(annual_files,pel_hb_fun)
```
...
r <- list.files(file.path(dir_M,'marine_threats/impact_acceleration/v2016/stressors/comm_fish/output/annual_catch'),pattern = '2014',full.names=T,recursive=T)%>%stack()%>%sum(.,na.rm=T)
...
data <- readRDS("/home/shares/ohi/marine_threats/impact_acceleration/v2016/stressors/comm_fish/int/catch_data_2014.rds")%>%
      rowwise()%>%
      mutate(catch = sum(LSF_CR,SSF_CR,IUU_CR,Discards_CR,na.rm=T))%>%
                  group_by(Seq)%>%  #group by cell number
                  summarise(cell_catch = sum(catch))
