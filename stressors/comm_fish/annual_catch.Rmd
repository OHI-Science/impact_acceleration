...
watson_data <- list.files(file.path(dir_M,'git-annex/impact_acceleration/stressors/comm_fish/data/catch'), pattern= "CatchPublic", full.names = TRUE)
...
dem_dest_fun <- function(file){
  ...
  #rasterize catch by swapping cell ids with 
      raster::subs(saup_rast, data, by = 1, which = 2, subsWithNA=TRUE, filename = file.path(dir_M, sprintf('git-annex/impact_acceleration/stressors/comm_fish/int/catch_annual_rasters/dem_dest/dem_dest_%s.tif', yr)), overwrite=TRUE)
}
...
dem_nd_hb_fun <- function(file){
  ...
  data <- readRDS(file)%>%
     ...
     raster::subs(saup_rast, data, by = 1, which = 2, subsWithNA=TRUE, filename = file.path(dir_M, sprintf('git-annex/impact_acceleration/stressors/comm_fish/int/catch_annual_rasters/dem_nondest_hb/dem_nondest_hb_%s.tif', yr)), overwrite = TRUE)
}
...
dem_nd_lb_fun <- function(file){
  ...
  data <- readRDS(file)%>%
     ...
     raster::subs(saup_rast, data, by = 1, which = 2, subsWithNA=TRUE, filename = file.path(dir_M, sprintf('git-annex/impact_acceleration/stressors/comm_fish/int/catch_annual_rasters/dem_nondest_lb/dem_nondest_lb_%s.tif', yr)), overwrite=TRUE)
}
...
pel_lb_fun <- function(file){
  ...
  data <- readRDS(file)%>%
     ...
     raster::subs(saup_rast, data, by = 1, which = 2, subsWithNA=TRUE, filename = file.path(dir_M, sprintf('git-annex/impact_acceleration/stressors/comm_fish/int/catch_annual_rasters/pel_lb/pel_lb_%s.tif', yr)), overwrite=TRUE)
}
...
pel_hb_fun <- function(file){
  ...
  data <- readRDS(file)%>%
     ...
     raster::subs(saup_rast, data, by = 1, which = 2, subsWithNA=TRUE, filename = file.path(dir_M, sprintf('git-annex/impact_acceleration/stressors/comm_fish/int/catch_annual_rasters/pel_hb/pel_hb_%s.tif', yr)), overwrite = TRUE)
}
...
data <- readRDS(file.path(dir_M, "git-annex/impact_acceleration/stressors/comm_fish/int/catch_annual_data/catch_data_2014.rds"))%>%
      rowwise()%>%
      mutate(catch = sum(LSF_CR,SSF_CR,IUU_CR,Discards_CR,na.rm=T))%>%
                  group_by(Seq)%>%  #group by cell number
                  summarise(cell_catch = sum(catch))
