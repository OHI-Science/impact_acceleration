...
Gapfill missing NPP raster data.  This is done by averaging the neighboring cells of raster cells with NA values and repeating until all ocean cells are gapfilled at the final resolution. 
...
writeRaster(r, file.path(dir_M, sprintf("/git-annex/impact_acceleration/stressors/comm_fish/int/npp_annual/annual_mean_npp_%s_gf.tif", yr)), overwrite=TRUE)
...
# relevant files
all <- list.files(file.path(dir_M,'git-annex/impact_acceleration/stressors/comm_fish/int/catch_annual_npp_rasters'), full.names = TRUE, recursive = TRUE)
ref_points <- read.csv("comm_fish_ref_points.csv")

# function to run through the 5 categories
scale_fun <- function(prs){ # prs <- "pel_hb"

    files <-  all[which(str_detect(all, prs))]
 cat(prs)
...
scale_fun(prs = "pel_hb")
scale_fun(prs = "pel_lb")
scale_fun(prs = "dem_dest")
scale_fun(prs = "dem_nondest_hb")
scale_fun(prs = "dem_nondest_lb")
```
