...
catch_npp_fun <- function(file,layer){
  ...
  writeRaster(c,filename = paste0(file.path(dir_M),'/git-annex/impact_acceleration/stressors/comm_fish/int/catch_annual_npp_rasters/',layer,'/',layer,'_',yr,'.tif'),overwrite=T)  
...
dem_d_files <- list.files(file.path(dir_M,'git-annex/impact_acceleration/stressors/comm_fish/int/catch_annual_rasters/dem_dest'),full.names=T)[54:65]
dem_nd_hb    <-list.files(file.path(dir_M,'git-annex/impact_acceleration/stressors/comm_fish/int/catch_annual_rasters/dem_nondest_hb'),full.names=T)[54:65]
dem_nd_lb    <-list.files(file.path(dir_M,'git-annex/impact_acceleration/stressors/comm_fish/int/catch_annual_rasters/dem_nondest_lb'),full.names=T)[54:65]
pel_hb    <-list.files(file.path(dir_M,'git-annex/impact_acceleration/stressors/comm_fish/int/catch_annual_rasters/pel_hb'),full.names=T)[54:65]
pel_lb    <-list.files(file.path(dir_M,'git-annex/impact_acceleration/stressors/comm_fish/int/catch_annual_rasters/pel_lb'),full.names=T)[54:65]
...
#list of lists
all <- list.files(file.path(dir_M,'git-annex/impact_acceleration/stressors/comm_fish/int/catch_annual_npp_rasters'),full.names=T,recursive = T)
  ...
  for (i in 1:12){
    ...
    yr = substr(files[i],nchar(files[i])-7,nchar(files[i])-4)
          ...
          resample(ocean,method = 'ngb')%>%
                     writeRaster(filename = paste0(file.path(dir_M),'/git-annex/impact_acceleration/stressors/comm_fish/final/', layer,'/',layer,'_',yr,'.tif'),format="GTiff", overwrite=TRUE)
  }
}
```
