...
gf <- raster(file.path(dir_M,'/git-annex/impact_acceleration/stressors/comm_fish/int/npp_gf.tif'))
...
catch_npp_fun <- function(file, layer){ # file = dem_d_files[12],  layer = 'dem_dest'
  ...
  writeRaster(c, filename = file.path(dir_M, sprintf('git-annex/impact_acceleration/stressors/comm_fish_v3.0/int/catch_annual_npp_rasters/%s/%s_%s.tif', layer, layer, yr)), overwrite=TRUE) 
  
#  raster(file.path(dir_M, sprintf('git-annex/impact_acceleration/stressors/comm_fish/int/catch_annual_npp_rasters/%s/%s_%s.tif', layer, layer, yr)))
}
...
dem_d_files <- list.files(file.path(dir_M,'git-annex/impact_acceleration/stressors/comm_fish_v3.0/int/catch_annual_rasters/dem_dest'), full.names=TRUE, pattern = "gf")
dem_d_files <- dem_d_files[grep(years_filter, dem_d_files)]
#dem_d_files <- dem_d_files[-grep("all_catch", dem_d_files)]

dem_nd_hb    <-list.files(file.path(dir_M, 'git-annex/impact_acceleration/stressors/comm_fish_v3.0/int/catch_annual_rasters/dem_nondest_hb'), full.names=TRUE, pattern = "gf")
dem_nd_hb <- dem_nd_hb[grep(years_filter, dem_nd_hb)]

dem_nd_lb    <-list.files(file.path(dir_M, 'git-annex/impact_acceleration/stressors/comm_fish_v3.0/int/catch_annual_rasters/dem_nondest_lb'), full.names=TRUE, pattern = "gf")
dem_nd_lb <- dem_nd_lb[grep(years_filter, dem_nd_lb)]

pel_hb    <-list.files(file.path(dir_M, 'git-annex/impact_acceleration/stressors/comm_fish_v3.0/int/catch_annual_rasters/pel_hb'), full.names=TRUE, pattern = "gf")
pel_hb <- pel_hb[grep(years_filter, pel_hb)]

pel_lb    <-list.files(file.path(dir_M, 'git-annex/impact_acceleration/stressors/comm_fish_v3.0/int/catch_annual_rasters/pel_lb'), full.names=TRUE, pattern = "gf")
...
catch_npp <- list.files(file.path(dir_M,'git-annex/impact_acceleration/stressors/comm_fish_v3.0/int/catch_annual_npp_rasters'), full.names = TRUE, recursive = TRUE)
...
all <- list.files(file.path(dir_M,'git-annex/impact_acceleration/stressors/comm_fish_v3.0/int/catch_annual_npp_rasters'), full.names = TRUE, recursive = TRUE)
...
for(layer in layers){
  ...
  refs <- read.csv("github/impact_acceleration/stressors/comm_fish_v3.0/comm_fish_ref_points.csv") %>%
    dplyr::mutate(ref_point = ifelse(pressure == layer, quant, ref_point))
  write.csv(refs, "github/impact_acceleration/stressors/comm_fish_v3.0/comm_fish_ref_points.csv", row.names=FALSE)
...
ref_points <- read.csv("github/impact_acceleration/stressors/comm_fish_v3.0/comm_fish_ref_points.csv")
    ...
    for(ind_rast in files){ # ind_rast=files[1]
        ...
        raster::raster(ind_rast) %>%
          ...
          raster::resample(ocean, method = 'ngb') %>%
            raster::mask(ocean,
                         filename = file.path(dir_M, sprintf('git-annex/impact_acceleration/stressors/comm_fish_v3.0/final/%s/%s_%s_rescaled_mol.tif', prs, prs, yr)), overwrite=TRUE)  
        # check:
        # tmp <- raster(file.path(dir_M, "git-annex/impact_acceleration/stressors/comm_fish_v3.0/final/pel_hb/pel_hb_2003_rescaled_mol.tif"))
        # plot(tmp)
    }
}
...
```{r}
dem_dest <- raster(file.path(dir_M, ('git-annex/impact_acceleration/stressors/comm_fish_v3.0/final/dem_dest/dem_dest_2014_rescaled_mol.tif')))
low_res_dem_dest <- aggregate(dem_dest, fact = 25, fun = mean, na.rm = T)
low_res_dem_dest[low_res_dem_dest == 0] <- NA

tm_shape(log(low_res_dem_dest))+
  tm_raster(palette = "-RdYlBu",
            legend.reverse = T, 
            style = 'quantile',
            legend.format = function(x){as.character(round(exp(x),3))})
```

```{r}
dem_nondest_hb <- raster(file.path(dir_M, ('git-annex/impact_acceleration/stressors/comm_fish_v3.0/final/dem_nondest_hb/dem_nondest_hb_2014_rescaled_mol.tif')))
low_res_dem_nondest_hb <- aggregate(dem_nondest_hb, fact = 25, fun = mean, na.rm = T)
low_res_dem_nondest_hb[low_res_dem_nondest_hb == 0] <- NA

tm_shape(log(low_res_dem_nondest_hb))+
  tm_raster(palette = "-RdYlBu",
            legend.reverse = T, 
            style = 'quantile',
            legend.format = function(x){as.character(round(exp(x),3))})
...
```{r}
dem_nondest_lb <- raster(file.path(dir_M, ('git-annex/impact_acceleration/stressors/comm_fish_v3.0/final/dem_nondest_lb/dem_nondest_lb_2014_rescaled_mol.tif')))
low_res_dem_nondest_lb <- aggregate(dem_nondest_lb, fact = 25, fun = mean, na.rm = T)
low_res_dem_nondest_lb[low_res_dem_nondest_lb == 0] <- NA

tm_shape(log(low_res_dem_nondest_lb))+
  tm_raster(palette = "-RdYlBu",
            legend.reverse = T, 
            style = 'quantile',
            legend.format = function(x){as.character(round(exp(x),3))})
...
```{r}
pel_hb <- raster(file.path(dir_M, ('git-annex/impact_acceleration/stressors/comm_fish_v3.0/final/pel_hb/pel_hb_2014_rescaled_mol.tif')))
low_res_pel_hb <- aggregate(pel_hb, fact = 25, fun = mean, na.rm = T)
low_res_pel_hb[low_res_pel_hb == 0] <- NA

tm_shape(log(low_res_pel_hb))+
  tm_raster(palette = "-RdYlBu",
            legend.reverse = T, 
            style = 'quantile',
            legend.format = function(x){as.character(round(exp(x),3))})
...
```{r}
pel_lb <- raster(file.path(dir_M, ('git-annex/impact_acceleration/stressors/comm_fish_v3.0/final/pel_lb/pel_lb_2014_rescaled_mol.tif')))
low_res_pel_lb <- aggregate(pel_lb, fact = 25, fun = mean, na.rm = T)
low_res_pel_lb[low_res_pel_lb == 0] <- NA

tm_shape(log(low_res_pel_lb))+
  tm_raster(palette = "-RdYlBu",
            legend.reverse = T, 
            style = 'quantile',
            legend.format = function(x){as.character(round(exp(x),3))})
