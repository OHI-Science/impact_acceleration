---
title: "wastewater: analyzing logged data"
...
ww_mol_extend <- raster(file.path(dir_M, "git-annex/land-based/wastewater/output/stressor/global_effluent_2015_all_raw_mol_nonegs_clean.tif"))

raster::calc(ww_mol_extend, function(x){log(x+1)}, 
       filename = file.path(dir_M, "git-annex/land-based/wastewater/output/log_data/stressor/global_effluent_2015_all_raw_mol_log.tif"), 
...
ww_log <- raster::raster(file.path(dir_M, "git-annex/land-based/wastewater/output/log_data/stressor/global_effluent_2015_all_raw_mol_log.tif"))

## find and save 99.99th quantile across raster
  quant9999 <-  quantile(ww_log, .9999)
  
  quant_df <- data.frame(stressor = "wastewater", quant = quant9999)

write.csv(quant_df, 
          here("projects/wastewater/log_data/quant.csv"),
                               row.names = FALSE)

## fertilizer/nutrient
quantiles <- read.csv(here("projects/wastewater/log_data/quant.csv"))
...
raster::calc(ww_log, fun=function(x){ifelse(x>ref_point, 1, x/ref_point)},
       filename = file.path(dir_M, "git-annex/land-based/wastewater/output/log_data/stressor/global_effluent_2015_mol_log_rescaled.tif"), 
       overwrite=TRUE, progress="text")


ww <- raster::raster(file.path(dir_M, "git-annex/land-based/wastewater/output/log_data/stressor/global_effluent_2015_mol_log_rescaled.tif"))
plot(ww)
zoom(ww)
```
...
foreach(row = 1:dim(vulnerability_clean)[1], .packages="dplyr") %dopar%{ # row=1
  ...
  stress_rast <- file.path(dir_M, "git-annex/land-based/wastewater/output/log_data/stressor/global_effluent_2015_mol_log_rescaled.tif")
  ...
  raster::overlay(combo_stack, fun=function(x,y){(x*y*vuln)}, 
          filename = file.path(dir_M, sprintf("git-annex/land-based/wastewater/output/log_data/hab_stressor_combo/global-effluent__%s__2015.tif", 
...
combos <- list.files(file.path(dir_M, "git-annex/land-based/wastewater/output/log_data/hab_stressor_combo"), full=TRUE)


stress_stack <- raster::stack(combos)
  
raster::calc(stress_stack, fun=sum, na.rm=TRUE,
      filename=file.path(dir_M, "git-annex/land-based/wastewater/output/log_data/tmp/summed_raster_global-effluent_2015.tif"), 
      overwrite=TRUE, progress="text")

  summed_rast <- raster::raster(file.path(dir_M, "git-annex/land-based/wastewater/output/log_data/tmp/summed_raster_global-effluent_2015.tif"))
  
  raster::overlay(summed_rast, hab_num, fun=function(x,y){x/y}, 
                  filename = file.path(dir_M, "git-annex/land-based/wastewater/output/log_data/tmp/summed_raster_global-effluent_2015_rescaled.tif"), overwrite=TRUE, progress="text")
...
```{r}
 ...
 rescaled <- raster::raster(file.path(dir_M, "git-annex/land-based/wastewater/output/log_data/tmp/summed_raster_global-effluent_2015_rescaled.tif"))

 # mask data and save
 raster::mask(rescaled, ocean,
                 filename=file.path(dir_M, "git-annex/land-based/wastewater/output/log_data/stressor_impact/global-effluent_2015.tif"),
                 overwrite=TRUE, progress="text")
 

 final_ww <- raster::raster(file.path(dir_M, "git-annex/land-based/wastewater/output/log_data/stressor_impact/global-effluent_2015.tif"))
...
ww <- list.files(file.path(dir_M, "git-annex/land-based/wastewater/output/log_data/stressor_impact"), full=TRUE)
ww <- raster(ww)
#chi_stack <- stack(chi)
...
write.csv(impacts_all, here("projects/wastewater/log_data/eez_3nm.csv"), row.names=FALSE)
...
impacts <- read.csv(here("projects/wastewater/log_data/eez_3nm.csv"))
